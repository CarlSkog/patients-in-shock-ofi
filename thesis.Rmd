...
```{r ofitable, echo=FALSE, results='asis'}
library(kableExtra)

ofi_table %>%
  kbl(
    col.names = c("BE Shock Class", "OFI Category", "Count", "Percentage (%)"),
    caption = "Counts and Percentages of OFI Broad Categories by BE Shock Class"
  ) %>%
  kable_classic(full_width = FALSE) %>%
  row_spec(0, bold = TRUE) %>% # Optional: Makes header bold
  group_rows(index = table(ofi_table$BE_class)) # Automatically merges BE_class rows

```

```{r combined_ofitable, echo=FALSE, results='asis'}
library(kableExtra)
...
combined_table %>%
  kbl(
    ...
    caption = "Counts and Percentages of OFI Broad Categories by BE and SBP Shock Classes"
  ) %>%
  kable_classic(full_width = FALSE) %>%
  row_spec(0, bold = TRUE) # Optional: Makes header bold
...
The results showed significant lower risk for 
